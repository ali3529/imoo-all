...
axios.interceptors.response.use(response => response, error => {
  ...
  if (status === 400) {
    serverError(error.response, error.response.data.error)
  }
  if (status === 429) {
    serverError(error.response, error.response.data.message)
  }
  if (status >= 500) {
    serverError(error.response, i18n.t('error_alert_text'))
  }
...
async function serverError (response, message) {
