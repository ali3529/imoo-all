...
class ApiController extends Controller
{
   ...
   public function categories(Request $request)
   {
    ...
    {
        $cities = collect(City::ofCountry($request->get('countryId', 0))
            ->whereRaw(
                'LOWER(`name`) like ?',
        ...
        $cities2 = collect(City::ofCountry($request->get('countryId', 0))
            ->whereRaw(
                'LOWER(`name`) like ?',
                ['%' . strtolower($request->get('search', '')) . '%']
            )
            ...
            $merge = $cities->merge($cities2);
            $unique = $merge->unique();


        return Response::success('', $unique->values()->all());
